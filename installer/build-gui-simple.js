...
class SimpleGUIInstallerBuilder {
    constructor() {
    ...
    async build() {
        try {
            console.log('\n🔨 BUILDING SIMPLE GUI CUSTOM INSTALLER\n');
            ...
            console.log('\n✅ SIMPLE GUI INSTALLER BUILD COMPLETED!\n');
    ...
    async buildInstaller() {
        console.log('🔨 Building simple GUI installer executable...');
        
        try {
            // Build with pkg
            execSync('npx pkg main-gui-simple.js --targets node18-win-x64 --output Inferno-Console-Installer.exe', {
                cwd: this.rootDir,
                stdio: 'inherit'
            });
            
            console.log('✅ Simple GUI installer executable built');
        } catch (error) {
...
// Run the build
const builder = new SimpleGUIInstallerBuilder();
